...
implements MapADT<K,V> {
    ...
    public V get(K searchKey) {
    	if (searchKey.equals(this.key)) {
    		return this.value;
    	}
    	
    	if (searchKey.compareTo(this.key) < 0){
    		if (left.isEmpty()) {
    			return null;
    		}
		 ...
		 RL: double left rotation (a right rotation at p followed
	       ...
	       */
		   ...
		   if (this.key.compareTo(parent.key) < 0 &&
				   parent.key.compareTo(gran.key) < 0) {
			   rotateRight(gran);
		   }
		   ...
		   //System.out.println("comparison " + parent.key.compareTo(gran.key));
		   //compares LR
		   else if (this.key.compareTo(parent.key) > 0 && 
				   parent.key.compareTo(gran.key) < 0) {
			   rotateLeft(parent);
		   ...
		   //Compares RL
				   ...
				   gran.key.compareTo(parent.key) < 0) {
			   rotateRight(parent);
			   rotateLeft(gran);
		   }
